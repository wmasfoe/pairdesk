...
- [x] **QUIC 传输基座**：tokio+quinn，跨进程可靠传输验证通过
- [x] **打洞落地**：relay 信令交换端点 + QUIC 承载完整会话，四链路(直连/中继/QUIC/信令)全绿
- [x] **自动择一**：viewer 无感择优——QUIC 打洞优先，失败自动降级中继兜底(auto-e2e 8/8)
- [ ] 三路全自动（把同网 TCP 直连也并入自动择一，含精确竞速优先）
